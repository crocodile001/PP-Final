...
int main(int argc, char **argv)
{
    ...
    if(rank == 0){
        ...
        for (int source = 1; source < size; source++){
            ...
            MPI_Irecv(&img[offset][0][0], rows * ny * 3, MPI_UNSIGNED_CHAR, source, 1, MPI_COMM_WORLD, &requests[source-1]);
            offset = offset + rows;
    ...
    if(rank == 0){
        ...
        MPI_Request req;
        MPI_Isend(&img[start][0][0], dx * ny * 3, MPI_UNSIGNED_CHAR, 0, 1, MPI_COMM_WORLD, &req);
        MPI_Wait(&req, &status);
